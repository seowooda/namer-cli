...
import { NamerConfig } from '../utils/configLoader.js';
...
async function handleSingleFile(selectedName: string, config: NamerConfig) {
  const fileOpts = await ui.askSingleFileOpts();
  const ext = fileOpts.extension === '직접입력' ? fileOpts.customExt : fileOpts.extension;
...
async function handleBundle(selectedName: string, config: NamerConfig) {
  const bundleOpts = await ui.askBundleOpts();
  const componentName = changeCase.pascalCase(selectedName);
  const folderPath = path.resolve(process.cwd(), bundleOpts.folder, componentName);
  ...
  // Index
  fs.writeFileSync(path.join(folderPath, `index.ts`), generateCode(componentName, 'index', config));
